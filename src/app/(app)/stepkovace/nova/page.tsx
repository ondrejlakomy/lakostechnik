...
export default function NovyStepkovacPage() {
  const router = useRouter();
  const [form, setForm] = useState({ name: "", internalCode: "", serialNumber: "", note: "" });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      toast.success("Štěpkovač vytvořen");
  ...
  return (
    <>
      <PageHeader title="Nový štěpkovač" />
      <form onSubmit={handleSubmit} className="max-w-2xl bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        <FormField label="Název" name="name" value={form.name} onChange={handleChange} required placeholder="Název štěpkovače" />
        <FormField label="Interní kód" name="internalCode" value={form.internalCode} onChange={handleChange} placeholder="Volitelně" />
        <FormField label="Sériové číslo" name="serialNumber" value={form.serialNumber} onChange={handleChange} placeholder="Volitelně" />
