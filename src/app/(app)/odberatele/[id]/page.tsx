...
export default function EditOdberatelPage() {
  const router = useRouter();
  const params = useParams();
  const [form, setForm] = useState({ name: "", ico: "", address: "", contactPerson: "", phone: "", email: "", note: "", active: true });
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    fetch(`/api/odberatele/${params.id}`)
      .then((r) => r.json())
      .then((d) => {
        setForm({ name: d.name, ico: d.ico || "", address: d.address || "", contactPerson: d.contactPerson || "", phone: d.phone || "", email: d.email || "", note: d.note || "", active: d.active });
        setLoading(false);
      });
  }, [params.id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    const res = await fetch(`/api/odberatele/${params.id}`, {
      method: "PUT",
  ...
  return (
    <>
      <PageHeader title="Upravit odběratele" />
      <form onSubmit={handleSubmit} className="max-w-2xl bg-white rounded-xl border border-gray-200 p-6 space-y-4">
        ...
        <FormField label="IČO" name="ico" value={form.ico} onChange={handleChange} />
